...
internal sealed class ProjectionPollingProcessor<TProjection>(
    ILogger<ProjectionPollingProcessor<TProjection>> logger,
...
where TProjection : Projection
{
    ...
    {
        // SQL Datatime min value is 1753-01-01
        if (startFrom < new DateTime(1753, 1, 1))
        {
            startFrom = new DateTime(1753, 1, 1);
        }
        ...
        var subscriptionExists = await dbConnection.ExecuteScalarAsync<bool>(
            ...
            new { StreamType = streamType, Name = projection.Name }
        );
        ...
        if (!subscriptionExists)
        {
            ...
            long? firstClusterKey = (await dbConnection.QueryFirstOrDefaultAsync<long?>(
                "SELECT TOP 1 ClusterKey FROM Streams WHERE StreamType = @StreamType ORDER BY ClusterKey",
                new { StreamType = streamType }
            ) - 1) ?? -1;

            await dbConnection.ExecuteAsync(
                "INSERT INTO Subscriptions (StreamType, Name, Continuation) VALUES (@StreamType, @Name, @Continuation)",
                new { StreamType = streamType, Name = projection.Name, Continuation = firstClusterKey }
            );
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            if (lastEvent == -1)
            {
                ...
                long? lastClusterKey = await dbConnection.QueryFirstOrDefaultAsync<long?>(
                    ...
                    new { StreamType = streamType, Created = startFrom }
                );
            ...
            var events = await dbConnection.QueryAsync<SqlEventData>(
                "SELECT TOP 1 * FROM Streams WHERE StreamType = @StreamType AND ClusterKey = @ClusterKey ORDER BY ClusterKey",
                new { StreamType = streamType, ClusterKey = lastEvent }
            );
            ...
            await dbConnection.ExecuteAsync(
                ...
                new { Continuation = lastEvent, StreamType = streamType, Name = projection.Name }
            );
